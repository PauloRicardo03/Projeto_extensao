...
# CSS para esconder status e rotacionar vídeo
st.markdown(
    """
    <style>
...
class MeuProcessadorDeVideo(VideoProcessorBase):
    def __init__(self):
        ...
        self.buffer_G = []
        self.buffer_B = []
        self.buffer_size = 75  # reduzido para resposta mais rápida
    ...
    def recv(self, frame):
        with self.lock:
            ...
            if mean_red_value > RED_THRESHOLD:
                ...
                # calcula bpm a cada 15 frames
                if self.contador_frames % 15 == 0:
                    bpm_bruto = self.calcula_bpm()
                    if bpm_bruto > 40:
                        self.bpm = (
                            bpm_bruto
                            if self.bpm == 0
                            else bpm_bruto * self.alpha_suavizacao
...
# ----------------------------------
# Configuração da câmera (otimizada)
# ----------------------------------
video_constraints = {
    "video": {
